...
//       const cloud = response.data?.data || {}; // cloudinary result

//       const imageUrl =
//         cloud.url ||
//         cloud.secure_url ||
//         cloud.data?.url ||
//         cloud.data?.secure_url ||
//         "";

//       if (!imageUrl) {
//         alert("Image URL not found in upload response!");
//         setImageLoading(false);
//         return;
//       }
...
const EditProductAdmin = ({ close, data: propsData, fetchProductData }) => {
  ...
  const handleUploadImage = async (e) => {
    ...
    try {
      ...
      // 2. Log the *actual* response data to the console
      console.log("UPLOAD RESPONSE FROM SERVER:", response.data);

      // 3. Get the response data object.
      // The URL is very likely in response.data, NOT response.data.data
      const responseData = response.data || {};

      // 4. Find the URL in the most common places
      const imageUrl =
        responseData.url || // Is it response.data.url?
        responseData.secure_url || // Is it response.data.secure_url? (Common for Cloudinary)
        responseData.data?.url || // Just in case it *is* nested
        responseData.data?.secure_url || //
        ""; // Default to empty string if not found

      // 5. Check if we actually found a URL
      if (!imageUrl) {
        console.error(
          "Error: Image URL not found in server response.",
          responseData
        );
        alert(
          "Error: Image URL not found in server response. Check the console."
        );
        setImageLoading(false);
        return;
      }

      // 6. If we found it, update the state
      setData((prev) => ({
        ...prev,
        image: [...prev.image, imageUrl],
      }));

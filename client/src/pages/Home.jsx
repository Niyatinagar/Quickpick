      ...
      try {
        if (userOrders && userOrders.length > 0) {
          ...
          recentOrders.forEach((order) => {
            // Iterate through ONLY the recent orders
            order.products.forEach((item) => {
              // Check if productId is populated and has a category array
              // Assumes the backend API populates order.products.productId with product details including category
              if (
                ...
                item.productId.category.forEach((catId) => {
                  if (catId) {
                    // Ensure the ID is not null/undefined
                    categoryIds.add(catId.toString()); // Add the category ID (convert ObjectId to string if needed)
                  }
                });
              } else if (item.product_details?.category) {
          ...
          if (categoryIds.size > 0) {
            ...
            const categoryFetchPromises = Array.from(categoryIds).map((catId) =>
              Axios({
                ...
                .catch((err) => {
                  console.error(
                    `Error fetching products for category ${catId}:`,
                    err
                  );
                  return []; // Return empty array on error for this category
                })
            ...
            const uniqueProducts = Array.from(
              new Map(fetchedProducts.map((p) => [p._id, p])).values()
            );
            fetchedProducts = uniqueProducts.slice(0, 12); // Limit to 12 recommendations
          } else {
            // Fallback if no category IDs found in recent orders (will trigger random logic below)
  ...
  return (
    <section className="bg-white pb-8">
      ...
      <div className="container mx-auto">
        <div
          className={`w-full h-full min-h-48 bg-slate-200 rounded ${
      ...
      {/* Category Grid */}
      <div className="container mx-auto px-4 my-6 grid grid-cols-4 sm:grid-cols-5 md:grid-cols-8 lg:grid-cols-10 gap-3 md:gap-4">
        {" "}
        {/* Adjusted grid cols and gap */}
        {loadingCategory
          ? new Array(10).fill(null).map((_, index) => {
              // Reduced loading items
              return (
                <div
                  key={index + "loadingcategory"}
                  className="bg-slate-100 rounded-lg p-2 h-24 md:h-28 flex flex-col items-center justify-center gap-2 shadow animate-pulse"
                >
          ...
          : categoryData.map((cat) => {
              return (
                ...
                >
                  <div
                    className="w-16 h-16 md:w-20 md:h-20 p-1 bg-white rounded-full shadow-sm flex items-center justify-center overflow-hidden mb-1
"
                  >
                    <img
                      src={cat.image}
                  ...
                  <p className="text-xs md:text-sm text-center font-medium text-slate-600 capitalize">
                    {cat.name}
                  </p>
                </div>
              );
